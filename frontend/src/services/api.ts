...
// Use intersection type to add the _retry property to AxiosRequestConfig
...
//   timeout: 10000,
  headers: {
    ...
    // 'Allow-Control-Allow-Origin': '*', // This header is not valid, remove it
  },
});
...
export const login = async (email: string, password: string) => {
  try {
    const { data } = await apiClient.post<{
      data: { access_token: any; refresh_token: any; role: any; message: any; };
      access_token: string;
